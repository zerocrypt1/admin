...
// Import CSS
// Reuse Desktop1 styles with potential overrides
...
function Desktop3() {
  const [formData, setFormData] = useState({
    name: '',
    occupation: [], // Array in state for multi-select
    phoneNumber: '',
  ...
  useEffect(() => {
    ...
    if (location.search) {
      ...
      // Extract form ID separately to use for updates
      if (params.get('_id')) {
      ...
      try {
        ...
        console.error("Error parsing timming data:", error);
        initialData.timming = [];
      }
      
      // Handle coordinates (decode and parse JSON)
      try {
        ...
        if (coordinatesEncoded) {
          ...
          // Set manual lat/lng to display the current values
  ...
  const handleSubmit = async (e) => {
    ...
    console.log("Updating form data:", submissionData);
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Navigate back to directory after successful update
        setTimeout(() => {
          navigate('/desktop2');
        }, 1500);
      } else {
                ...
                <div className="desktop3-phone-note">
                  {formData.phoneNumber !== originalPhoneNumber ? (
                    <span className="desktop3-warning">
                      ⚠️ Changing the phone number will require re-verification in production.
                    </span>
                  ) : (
